...
function t_animate__animateChainItemsOnScroll(t, a, e) {
    ...
    for (var d = 0; d < i.length; d++) {
        ...
        if (!(c < e)) break;
        c != o && (l.addClass("t-animate__chain_first-in-row"), s = ++r, o = c);
        var f = .16 * s + m + _;
        l.css("transition-delay", f + "s"), l.removeClass("t-animate_wait"), l.addClass("t-animate_started"), l.attr("data-animate-start-time", Date.now() + 1e3 * f), l[0] == i.last()[0] && t_animate__checkSectionButtonAnimation(n, f), c == o && s++, i.splice(d, 1), t[a].itemsOffsets.splice(d, 1), d--
    }
    ...
    t.each(function() {
        ...
        if (!($(this).offset().top < o - 500) && 0 < a.length) {
            ...
            if (s) {
                ...
                t.attr("data-animate-number-count", a), t_animate__changeNumberOnZero(t, i, s), t.find("span:not(.t-animate__number):first").each(function() {
                    $(this).attr("style", e)
                })
            }
        }
...
function t_animate__animateNumbers(t) {
    t.each(function() {
        var n = $(this),
            t = n.attr("data-animate-number-count"),
            a = [];
        if (n.find("span:not(.t-animate__number):first").each(function() {
                a = $(this).attr("style")
            }), t) {
            var e = t.match(/\d+\.\d+|\d+\,\d+/g),
                i = t.match(/\d+/g),
                s = t.replace(/(\d)(?= \d) /g, "$1").match(/\d+/g),
                r = 0,
                o = !1;
            n.removeAttr("data-animate-number-count"), null != e && (o = -1 != e[0].indexOf(",")), 2 == s.length && (r = s[1].length, s = +s.join("."), !0);
            var m = n.find(".t-animate__number").text();
            $({
                animateCounter: m
            }).animate({
                animateCounter: s
                ...
                step: function(t) {
                    var a = Math.pow(10, r),
                        e = t ? (Math.round(this.animateCounter * a) / a).toFixed(r) + "" : Math.floor(this.animateCounter) + "";
                    e = 1 < i.length ? e.replace(/(\d)(?=(\d{3})+([^\d]|$))/g, "$1 ") : e, o ? n.find(".t-animate__number").text(e.replace(/\./g, ",")) : n.find(".t-animate__number").text(e)
                },
                complete: function() {
                    n.html(t), n.find("span").each(function() {
                        $(this).attr("style", a)
                    })
                }
            })
...
function t_animate__setAnimationState(t, a, e) {
    ...
    t.each(function() {
        ...
        t_animate__setCustomAnimSettings(t, a, i), a < e && (t_animate__removeNoHoverClassFromBtns(t), t.addClass("t-animate_started"), t.is('[data-animate-style="animatednumber"]') && t_animate__animateNumbers(t)), e <= a && t.addClass("t-animate_wait")
...
function t_animate__setCustomAnimSettings(t, a, e) {
    ...
    var o = t.attr("data-animate-duration");
